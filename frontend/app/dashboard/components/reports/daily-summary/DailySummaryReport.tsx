...
import { FileText, FileDown, Printer, X, BarChart3 } from 'lucide-react';
...
export default function DailySummaryReport({ date, onClose, data }: DailySummaryReportProps) {
    // Mock data based on the screenshot, same structure as Financial Summary
    const summaryData = {
        totalBuyIns: 0,
        totalCashOuts: 0,
        totalRake: 0,
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <h3 className="text-xl font-bold flex items-center gap-2 mb-6 text-slate-800">
                <span className="p-1 rounded bg-blue-100 text-blue-600"><BarChart3 className="w-5 h-5" /></span>
                Daily Summary — {date}
            </h3>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden mb-8">
                <div className="flex justify-between items-center p-4 border-b border-slate-100">
                    ...
                    <span className="font-semibold text-slate-800 h-9 flex items-center">{summaryData.totalBuyIns}</span>
                </div>
                <div className="flex justify-between items-center p-4 border-b border-slate-100">
                    <span className="text-slate-600 font-medium h-9 flex items-center">Total Cash-outs</span>
                    <span className="font-semibold text-slate-800 h-9 flex items-center">{summaryData.totalCashOuts}</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-green-100 border-b border-green-200">
                    ...
                    <span className="font-bold text-green-900 h-9 flex items-center">{summaryData.totalRake}</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-red-100 border-b border-red-200">
                    ...
                    <span className="font-semibold text-red-900 h-9 flex items-center">-{summaryData.casinoShare}</span>
                </div>
                <div className="flex justify-between items-center p-4 border-b border-slate-100">
                    <span className="text-slate-600 font-medium h-9 flex items-center">House Revenue</span>
                    <span className="font-semibold text-slate-800 h-9 flex items-center">{summaryData.houseRevenue}</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-red-100 border-b border-red-200">
                    <span className="text-red-900 font-medium h-9 flex items-center">Expenses</span>
                    <span className="font-semibold text-red-900 h-9 flex items-center">-{summaryData.expenses}</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-[#1e293b] text-white">
                    ...
                    <span className="font-bold text-lg h-9 flex items-center">{summaryData.netResult}</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-slate-50 p-6 rounded-lg border border-slate-100 text-center">
                    <div className="text-2xl font-bold text-slate-800 mb-1">{summaryData.tablesCount}</div>
                    <div className="text-xs text-slate-500 uppercase font-medium">Tables</div>
                </div>
                <div className="bg-slate-50 p-6 rounded-lg border border-slate-100 text-center">
                    <div className="text-2xl font-bold text-slate-800 mb-1">{summaryData.playersCount}</div>
                    <div className="text-xs text-slate-500 uppercase font-medium">Players</div>
                </div>
                <div className="bg-slate-50 p-6 rounded-lg border border-slate-100 text-center">
                    <div className="text-2xl font-bold text-slate-800 mb-1">{summaryData.hoursCount.toFixed(2)}</div>
                    <div className="text-xs text-slate-500 uppercase font-medium">Hours</div>
                </div>
                <div className="bg-slate-50 p-6 rounded-lg border border-slate-100 text-center">
                    <div className="text-2xl font-bold text-slate-800 mb-1">{summaryData.pointsCount}</div>
